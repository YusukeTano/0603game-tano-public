...
export class BackgroundSystem {
    ...
    renderSpaceStation(structure) {
        ...
        try {
            ...
            // メイン構造
            this.ctx.fillStyle = structure.color; // Canvas状態変更
            this.ctx.beginPath();
            ...
            // 中央コア
            this.ctx.fillStyle = 'rgba(255, 255, 255, 0.5)'; // Canvas状態変更
            this.ctx.beginPath();
    ...
    renderPlanet(structure) {
        ...
        try {
            ...
            if (structure.config.atmosphere) {
                this.ctx.globalAlpha *= 0.3; // ⚠️ 問題のglobalAlpha変更
            ...
            console.error('❌ Error in renderPlanet:', error);
        }
        ...
        try {
            ...
            if (structure.config.warp) {
                ...
                for (let i = 0; i < 5; i++) {
                    ...
                    this.ctx.globalAlpha = alpha; // ⚠️ 問題のglobalAlpha変更
            ...
            console.error('❌ Error in renderQuantumPortal:', error);
        }
    ...
    /**
     * Canvas描画状態完全リセット（キャラクター描画汚染防止）
     * @private
     */
    resetCanvasState() {
        try {
            // 🔧 Canvas状態完全初期化
            this.ctx.globalAlpha = 1.0;           // 透明度リセット
            this.ctx.globalCompositeOperation = 'source-over'; // 合成モードリセット
            
            // 🎨 色・スタイルリセット
            this.ctx.fillStyle = '#000000';        // 塗りつぶし色リセット
            this.ctx.strokeStyle = '#000000';      // 線色リセット
            this.ctx.lineWidth = 1;               // 線幅リセット
            this.ctx.lineCap = 'butt';            // 線端リセット
            this.ctx.lineJoin = 'miter';          // 線結合リセット
            this.ctx.miterLimit = 10;             // マイター制限リセット
            
            // ✨ エフェクトリセット
            this.ctx.shadowBlur = 0;              // ブラーリセット
            this.ctx.shadowColor = 'rgba(0, 0, 0, 0)'; // 影色リセット
            this.ctx.shadowOffsetX = 0;           // 影X座標リセット
            this.ctx.shadowOffsetY = 0;           // 影Y座標リセット
            
            // 📐 変換行列リセット
            this.ctx.setTransform(1, 0, 0, 1, 0, 0); // 変換行列完全リセット
            
            // 🎯 クリッピングリセット（もしあれば）
            // this.ctx.clip()でクリッピングが設定されている場合のリセット
            // beginPath()を呼ぶことでクリッピングパスもリセット
            this.ctx.beginPath();
            
            console.log('🔧✅ Canvas state completely reset - character rendering protection active');
            
        } catch (error) {
            console.error('❌ Canvas state reset failed:', error);
            // 最低限のリセット

...
export class ZombieSurvival {
    ...
    async initializeAudioSystem() {
        if (!this.audioSystem) {
            try {
                ...
                // 🚨 緊急安定化モード: EmergencyAudioSystemを直接使用
                console.log('🚨 緊急安定化モード: EmergencyAudioSystem直接使用...');
                ...
                if (result && result.success) {
                    console.log(`✅ AudioSystem: 緊急安定化モード - EmergencyAudioSystem初期化完了`);
                    ...
                    // 🚨 緊急モードではPhase 3 Manager統合をスキップ
                    console.log('ℹ️ 緊急安定化モード: Phase 3 Manager統合をスキップ');
                } else {
                    console.warn('⚠️ AudioSystem: 初期化で問題発生、フォールバック済み');
                }
                
            } catch (error) {
                console.error('❌ AudioSystem: 緊急安定化モード - 初期化失敗:', error);
                console.error('Error stack:', error.stack);
                
                // 🚨 緊急モードでは複雑なフォールバックチェーンを無効化
                console.log('🚨 緊急安定化モード: Audio System完全無効化');
                this.audioSystem = null;
            }
        } else {
    ...
    render() {
        ...
        this.ctx.globalAlpha = 1.0; // 🔧 星描画後のglobalAlphaリセット（重要）
        
        // 遠方の星雲エフェクト（BackgroundSystemに統合済み - 重複描画防止）
        ...
        // 🔧 Canvas状態安全確保（キャラクター描画汚染防止）
        this.ensureCleanCanvasState();
     ...
     * @private
     */
    ensureCleanCanvasState() {
        try {
            // 🔧 ゲームレベルでのCanvas状態検証・修正
            
            // 透明度の確認・修正
            if (this.ctx.globalAlpha !== 1.0) {
                console.warn(`⚠️ Canvas globalAlpha contaminated: ${this.ctx.globalAlpha} → 1.0`);
                this.ctx.globalAlpha = 1.0;
            }
            
            // シャドウエフェクトの確認・修正
            if (this.ctx.shadowBlur !== 0) {
                console.warn(`⚠️ Canvas shadowBlur contaminated: ${this.ctx.shadowBlur} → 0`);
            ...
            if (this.ctx.lineWidth !== 1) {
                console.warn(`⚠️ Canvas lineWidth contaminated: ${this.ctx.lineWidth} → 1`);
                this.ctx.lineWidth = 1;
            }
            
            // 合成モードの確認・修正
            if (this.ctx.globalCompositeOperation !== 'source-over') {
                console.warn(`⚠️ Canvas composite operation contaminated: ${this.ctx.globalCompositeOperation} → source-over`);
                this.ctx.globalCompositeOperation = 'source-over';
            }
            
            console.log('🔧✅ Game level Canvas state verification complete');
            
        } catch (error) {
            console.error('❌ Canvas state safety check failed:', error);
            // 最低限の安全確保
            this.ctx.globalAlpha = 1.0;
            this.ctx.shadowBlur = 0;
            this.ctx.lineWidth = 1;
     ...
     * @private
     */
    monitorCanvasStateDetailed() {
        if (!this.canvasStateMonitor) {
            this.canvasStateMonitor = {
                enabled: true,
                contaminations: [],
                lastCleanState: null
            };
        ...
        if (!this.canvasStateMonitor.enabled) return;
        ...
        try {
            ...
            // 初回時は基準状態として保存
            if (!this.canvasStateMonitor.lastCleanState) {
                this.canvasStateMonitor.lastCleanState = { ...currentState };
                return;
            }
            
            const last = this.canvasStateMonitor.lastCleanState;
            const contaminations = [];
            
            // 各状態の詳細チェック
            if (currentState.globalAlpha !== 1.0) {
                contaminations.push(`globalAlpha: ${currentState.globalAlpha} (should be 1.0)`);
            }
            
            if (currentState.shadowBlur !== 0) {
                contaminations.push(`shadowBlur: ${currentState.shadowBlur} (should be 0)`);
            }
            
            if (currentState.shadowColor !== 'rgba(0, 0, 0, 0)' && currentState.shadowColor !== 'transparent') {
                contaminations.push(`shadowColor: ${currentState.shadowColor} (should be transparent)`);
            }
            
            if (currentState.lineWidth !== 1) {
                contaminations.push(`lineWidth: ${currentState.lineWidth} (should be 1)`);
            }
            
            if (currentState.globalCompositeOperation !== 'source-over') {
                contaminations.push(`globalCompositeOperation: ${currentState.globalCompositeOperation} (should be source-over)`);
            }
            
            // 色の変化をチェック（青系色の特定監視）
            if (typeof currentState.fillStyle === 'string') {
                if (currentState.fillStyle.includes('rgba(100, 150, 255') || 
                    currentState.fillStyle.includes('#4444ff') ||
                    currentState.fillStyle.includes('blue')) {
                    contaminations.push(`fillStyle: ${currentState.fillStyle} (potential blue contamination)`);
                }
            }
            
            if (contaminations.length > 0) {
                // 🔧 スマート監視：同じ汚染の重複ログを防止
                const contaminationKey = contaminations.join('|');
                const now = Date.now();
                
                if (!this.canvasStateMonitor.lastLoggedContamination || 
                    this.canvasStateMonitor.lastLoggedContamination.key !== contaminationKey ||
                    now - this.canvasStateMonitor.lastLoggedContamination.time > 5000) { // 5秒経過後は再ログ
                    
                    console.error('🚨 Canvas State Contamination Detected:', {
                        contaminations: contaminations,
                        currentState: currentState,
                        renderingContext: this.getCurrentRenderingContext()
                    });
                    
                    this.canvasStateMonitor.lastLoggedContamination = {
                        key: contaminationKey,
                        time: now
                    };
                }
                
                // 汚染履歴に記録（ログ表示とは別に常に記録）
                this.canvasStateMonitor.contaminations.push({
                    timestamp: now,
                    contaminations: contaminations,
                    context: this.getCurrentRenderingContext()
                });
                
                // 汚染履歴の管理（最新50件のみ保持）
                if (this.canvasStateMonitor.contaminations.length > 50) {
                    this.canvasStateMonitor.contaminations = this.canvasStateMonitor.contaminations.slice(-50);
                }
            }
            
        } catch (error) {
            console.error('❌ Canvas state monitoring failed:', error);
        }
    }
    ...
    getCurrentRenderingContext() {
        ...
        for (let i = 1; i < Math.min(lines.length, 10); i++) {
            const line = lines[i];
            if (line.includes('render') && !line.includes('monitorCanvasState')) {
    ...
    /**
     * Canvas状態監視の有効/無効切り替え
     * @param {boolean} enabled - 監視の有効/無効
     */
    setCanvasStateMonitoring(enabled) {
        if (!this.canvasStateMonitor) {
            this.canvasStateMonitor = { enabled: false, contaminations: [], lastCleanState: null };
        }
        this.canvasStateMonitor.enabled = enabled;
        console.log(`🔧 Canvas state monitoring ${enabled ? 'enabled' : 'disabled'}`);
    }
     ...
     * @returns {Array} 汚染履歴
     */
    getCanvasContaminationHistory() {
        return this.canvasStateMonitor?.contaminations || [];
    }

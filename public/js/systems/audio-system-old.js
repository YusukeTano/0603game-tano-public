...
export class AudioSystem {
    constructor(game) {
        ...
        // 音量設定 (0.0 - 1.0) - 専門家推奨バランス
        this.volumeSettings = {
            master: 0.8,    // マスター音量
            bgm: 0.9,       // BGM音量 (0.6→0.9 大幅UP)
            sfx: 0.3        // 効果音音量 (0.7→0.3 大幅DOWN)
        };
    ...
    startBGM() {
        ...
        // 🔥 CRITICAL FIX: BGM開始時のAudioContext状態確認と復帰
        if (this.audioContext.state === 'suspended') {
            console.log('🎵 AudioSystem: Resuming suspended AudioContext before BGM start...');
            this.audioContext.resume().then(() => {
                console.log('🎵 AudioSystem: AudioContext resumed, starting BGM...');
                this.isBGMPlaying = true;
                this._startBGMExecution();
            }).catch(error => {
    ...
    _startBGMExecution() {
        ...
        const playChord = () => {
            // 🔥 CRITICAL FIX: AudioContext状態確認と復帰処理
            ...
            if (!this.audioContext || this.audioContext.state === 'suspended') {
                ...
                if (this.audioContext && this.audioContext.state === 'suspended') {
                    this.audioContext.resume().then(() => {
                        ...
                        setTimeout(playChord, 100); // 復帰後に再実行
                    }).catch(error => {
                        console.error('🎵 AudioSystem: Failed to resume AudioContext', error);
                        setTimeout(playChord, 1000); // 1秒後にリトライ
                    });
                    return;
                }
                if (!this.audioContext) {
                    console.error('🎵 AudioSystem: AudioContext is null, stopping BGM');
                    return;
                }
            }
            
            // 🔥 CRITICAL FIX: 動的フェーズ更新とテンポ再計算（例外処理強化）
            try {
                if (this.game.stageSystem && this.game.stageSystem.getStageInfo) {
                    const stageInfo = this.game.stageSystem.getStageInfo();
            ...
            try {
                currentChord.forEach((freq, index) => {
                    try {
                        ...
                        console.error('🎵 AudioSystem: Failed to create oscillator', { freq, index, error: oscError });
                    }
                });
            } catch (chordError) {
                console.error('🎵 AudioSystem: Failed to process chord', { currentChord, error: chordError });
            }
            ...
            // 🔥 CRITICAL FIX: 確実な継続実行（例外処理付き）
            try {
                setTimeout(playChord, chordDuration * 1000);
            } catch (error) {
                console.error('🎵 AudioSystem: setTimeout failed, using fallback', error);
                // フォールバック：1秒後に再試行
                setTimeout(() => {
                    try {
                        playChord();
                    } catch (e) {
                        console.error('🎵 AudioSystem: Fallback playChord failed', e);
                    }
                }, 1000);
            }
        };
        ...
        // ドラムパート（リズム）
        const playDrums = () => {
            // 🔥 CRITICAL FIX: ドラムパートのAudioContext状態確認
            if (!this.isBGMPlaying) return;
            
            // AudioContextの状態を確認
            if (!this.audioContext || this.audioContext.state === 'suspended') {
                console.warn('🥁 AudioSystem: AudioContext suspended in drums, skipping...');
                setTimeout(playDrums, 2000); // 2秒後に再試行
                return;
            }
    ...
    enableStage1Music() {
        ...
        // 🔥 CRITICAL FIX: ステージ1音楽開始時のAudioContext復帰
        if (this.audioContext && this.audioContext.state === 'suspended') {
            console.log('🎼 AudioSystem: Resuming AudioContext for Stage 1 music...');
            this.audioContext.resume().then(() => {
                console.log('🎼 AudioSystem: AudioContext resumed for Stage 1');
                this.stage1Active = true;
                this._enableStage1MusicExecution();
            }).catch(error => {
                console.error('🎼 AudioSystem: Failed to resume AudioContext for Stage 1', error);
            });
            return;
        }
    ...
    onSubPhaseChange(newSubPhase, config) {
        ...
        // 🔥 CRITICAL FIX: AudioContext状態確認
        if (!this.audioContext || this.audioContext.state === 'suspended') {
            console.warn('🎵 AudioSystem: AudioContext unavailable in onSubPhaseChange, skipping...');
            return;
        }
    ...
    playInstrumentLayer(instrument, config) {
        ...
        // 🔥 CRITICAL FIX: AudioContext状態確認
        if (!this.audioContext || this.audioContext.state === 'suspended') {
            console.warn('🎵 AudioSystem: AudioContext unavailable in playInstrumentLayer, skipping...');
            return;
        }
